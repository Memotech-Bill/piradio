...
int _snd_pcm_scope_pivumeter_open (snd_pcm_t * pcm, const char *name,
    ...
    snd_config_for_each (i, next, conf)
        ...
        if (strcmp (id, "log_file") == 0)
            {
            err = snd_config_get_string (n, &log_file);
            if (err < 0)
                {
                SNDERR ("Invalid type for %", id);
                return -EINVAL;  
    ...
    if (strlen (output_device_name) == 0)
        {
        fprintf (stderr, "No device specified, defaulting to phat-beat!\n");
        output_device_name = "phat-beat";
        }
